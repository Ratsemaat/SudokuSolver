...
def ImageToGrid(image):
    ...
    for index, line in enumerate(lines):
        ...
        if 1 < line.item(1) < 2:
            ...
            for el in vertical_lines:
                if abs(el.item(0) - line.item(0)) < suurus/50:
                    tooSimilar = True
                    break
            if tooSimilar == False:
                vertical_lines.append(line)
            ...
            for el in horizontal_lines:
                if abs(abs(el.item(0)) - abs(line.item(0))) < suurus/50:
                    tooSimilar = True
                    break
            if tooSimilar == False:
                horizontal_lines.append(line)
    ...
    for i in range(9):
        temp = []
        for j in range(9):
            area = whiteblackimg[cart_horizontal_lines[i][0]+suurus//200:cart_horizontal_lines[i + 1][0]-suurus//200,
                   cart_vertical_lines[j][0]+suurus//200: cart_vertical_lines[j + 1][0]-suurus//200]
