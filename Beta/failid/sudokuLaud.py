...
def TableShower(sudoku_table):
    ...
    window_width = 600
    master = Tk()
    ul=Canvas(height=window_width, width= window_width)

    for k in range(3):
        for l in range(3):
            laud = Frame(master=ul,highlightbackground="black",highlightthickness=1)
            for i in range(3):
                for j in range(3):
                    if sudoku_table[i+3*l][j+3*k] != "":
                        w = Label(master=laud, text=sudoku_table[i+3*l][j+3*k], width=2, height=1, font=("Ariel", 20),
                                  borderwidth=1, relief="raised")

                    else:
                        w = Label(master=laud, width=2, height=1, font=("Ariel", 20),
                                  borderwidth=1, relief="raised")
                    w.grid(row=i, column=j)
            laud.grid(column=k,row=l)
        ul.pack()
    master.mainloop()
...
def kontrollija(sudoku_table):
    ...
    for el in completeSet:
        for i in range(9):
            ...
            ctrud = 0
            for j in range(9):
                if sudoku_table[i][j]==el:
                    ctrlr+=1
                if sudoku_table[j][i]==el:
                    ctrud+=1
                if ctrlr > 1 or ctrud > 1:
                    return False
                if (i==0 or i==3 or i==6) and (j==0 or j==3 or j==6):
                    ctrb = 0
                    for k in range(3):
                        for l in range(3):
                            if sudoku_table[i+k][j+l]==el:
                                ctrb+=1
                            if ctrb>1:
                                return False
